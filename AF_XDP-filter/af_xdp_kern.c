...
static __always_inline __u32 stats_record_action(struct xdp_md *ctx,
						 __u32 action)
{
	if (k_tracing)
		bpf_printk("stats_record_action action=%d", action);
	...
	if (k_tracing)
		bpf_printk(
			"stats_record_action rec->rx_packets=%llu rec->rx_bytes=%llu",
...
static __always_inline int parse_tcp4hdr(struct hdr_cursor *nh, void *data_end,
...
static __always_inline int parse_udp4hdr(struct hdr_cursor *nh, void *data_end,
...
{
	if (k_tracing) {
		bpf_printk("fivetuple saddr=%08x daddr=%08x", f->saddr,
			   f->daddr);
		bpf_printk(" sport=%04x dport=%04x", f->sport, f->dport);
		bpf_printk(" protocol=%04x padding=%u", f->protocol,
			   f->padding);
	}
}
	...
	if (mapped) {
		...
		if (nh_type == bpf_htons(ETH_P_IP)) {
			...
			*proto = bpf_htons(ETH_P_IP);
			...
			if (k_tracing)
				bpf_printk("Unknown nh_type=0x%04x", nh_type);
			return stats_record_action(ctx, XDP_PASS);
		}

		if (k_tracing)
			bpf_printk("v_permit=%p", v_permit);
